...
- Home CTA `Download (PC Only)` and `Coba Gratis` use `/download`; navigation also exposes `Download`.
- `/download` uses ORDAL visual language and contains Windows/macOS buttons plus minimum practical spec.
- Download buttons use same `Button` and `stickerButtonPrimary` treatment as pricing: orange fill, hard shadow, hover lift, and icon motion.
- Windows/macOS buttons remain visually consistent when installer environment URLs are not configured.
- Minimum practical spec: Windows 10 64-bit+, macOS 12 Monterey+, Intel Core i3 or AMD Ryzen 3 equivalent, 4 GB RAM minimum, 8 GB recommended, 1 GB free storage, stable internet.
- Removed home spinning badge, repeated marquee noise, floating source ZIP preview, and excess hero motion.
- Tightened download spacing for 13-inch laptop at 100% zoom; full landing page still requires vertical scrolling because it contains multiple sections.
- `taste-skill` was used as visual review guidance, not runtime dependency.
- Production verification on `ordal-web.vercel.app` must happen after Vercel deploys latest `main`; cached old HTML can show previous CTA links until deployment completes.

## Repository sync

- Branch: `main`
- Remote: `origin/main`
- Latest commit: `60fd63f fix: tighten download section layout`
- Local and remote commit IDs match; no merge conflict or unpushed commit.
...
Branch: `main`
Base commit: `f480a0a`
Implementation commits: `2f34539`, `60fd63f` (pushed to `origin/main`).
